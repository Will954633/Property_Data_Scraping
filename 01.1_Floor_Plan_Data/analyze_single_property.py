"""
Script to analyze floor plans for a single property.
...
def analyze_single_property(address=None):
    """
    Analyze floor plans for a single property.
    ...
    try:
        ...
        if address:
            ...
            if not property_doc:
                logger.error("No properties with floor plans found in database")
                return
        ...
        # Save to output file
        output_file = f"output/floor_plan_analysis_{address.replace('/', '_').replace(' ', '_')}.json"
        ...
        logger.info("\n" + "=" * 80)
        response = input("\nSave this analysis to MongoDB? (yes/no): ").strip().lower()
        ...
        if "total_floor_area" in analysis_result:
            ...
            logger.info(f"Total Floor Area: {area.get('value')} {area.get('unit')} (confidence: {area.get('confidence')})")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        logger.info(f"Analyzing specific property: {address}")
        analyze_single_property(address)
    else:
        logger.info("No address specified - will analyze first property with floor plans")
