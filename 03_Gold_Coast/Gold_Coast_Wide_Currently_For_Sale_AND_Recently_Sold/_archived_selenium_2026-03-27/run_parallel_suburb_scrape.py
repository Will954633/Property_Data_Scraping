...
# Target market suburbs — these get full document tracking + enrichment pipeline.
# All suburbs now write to Gold_Coast database.
...
class ParallelSuburbScraper:
    ...
    def save_to_mongodb(self, property_data: Dict) -> bool:
        """
        Save property to Gold_Coast database (unified):
        1. Match on listing_url (same doc scraped previously)
        2. Fallback: match on complete_address against GIS doc
        3. If no match: insert new doc with listing_status: "for_sale"
        """
        try:
            ...
            # Log routing decision when suburb differs from search parameter
            ...
            # --- Fallback match: GIS complete_address (all suburbs are in Gold_Coast now) ---
            ...
            # --- Guard: don't insert sold properties as new for-sale docs ---
            ...
            if existing_doc:
                ...
                # Set listing_status to for_sale on update
