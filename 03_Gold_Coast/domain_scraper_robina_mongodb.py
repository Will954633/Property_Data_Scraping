#!/usr/bin/env python3
"""
Domain Property Scraper - Robina Priority (MongoDB Storage)
Specialized scraper for processing ONLY Robina addresses with direct MongoDB storage
Saves scraped data to MongoDB Gold_Coast database in 'robina' collection
"""
...
class RobinaScraperMongoDB:
    """Domain scraper for Robina - saves directly to MongoDB"""
    ...
    def __init__(self):
        ...
        self.total_workers = int(os.getenv('TOTAL_WORKERS', 20))
        ...
        self.input_collection = self.db['robina']  # Read addresses from here
        self.output_collection = self.db['robina']  # Write scraped data here
        ...
        print(f"Processing: ROBINA addresses only")
        print(f"Output: Gold_Coast.robina collection")
    ...
    def get_my_addresses(self) -> List[Dict]:
        """Get Robina addresses assigned to this worker"""
        print(f"\nRetrieving Robina addresses from MongoDB...")
        
        # Get all Robina addresses
        docs = list(self.input_collection.find({}, {
            'ADDRESS_PID': 1,
            'UNIT_TYPE': 1,
            'UNIT_NUMBER': 1,
            'STREET_NO_1': 1,
            ...
            'POSTCODE': 1,
            '_id': 1,
            'scraped_data': 1  # Check if already scraped
        }))
        
        addresses = []
        for doc in docs:
            # Skip if already has scraped data
            if doc.get('scraped_data'):
                continue
        ...
        # Sort by address_pid for consistent ordering
        ...
        print(f"Total unscraped Robina addresses: {total:,}")
    ...
    def save_to_mongodb(self, mongo_id, property_data: Dict) -> bool:
        ...
        try:
            ...
            if result.modified_count > 0:
                print(f"  ✓ Saved to MongoDB: Gold_Coast.robina (doc: {mongo_id})")
                return True
            else:
    ...
    def run(self):
        ...
        print(f"\n{'='*70}")
        print(f"ROBINA PRIORITY WORKER {self.worker_id} STARTING")
        print(f"{'='*70}\n")
        ...
        try:
            for i, addr_info in enumerate(addresses, 1):
                ...
                if property_data:
                    ...
                    # Save to MongoDB
                    if self.save_to_mongodb(addr_info['mongo_id'], property_data):
                        successful += 1
                    else:
                        failed += 1
                else:
        ...
        print(f"\n{'='*70}")
        print(f"ROBINA WORKER {self.worker_id} COMPLETE")
        print(f"{'='*70}")
