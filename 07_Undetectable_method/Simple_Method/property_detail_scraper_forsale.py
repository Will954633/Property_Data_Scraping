#!/usr/bin/env python3
"""
Property Detail Scraper for Domain.com.au - FOR-SALE PROPERTIES
...
def process_property_url(driver, url, index, total):
    """Process a single property listing URL using Selenium"""
    print(f"\n{'='*80}")
    print(f"Processing Property {index}/{total}")
    print(f"{'='*80}")
    print(f"URL: {url}")
    ...
    try:
        ...
        if property_data:
            ...
            for key, value in property_data.items():
                if key not in ['address', 'extraction_method', 'extraction_date', 'listing_url'] and value:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process Domain.com.au for-sale property listing URLs")
    ...
    print("\n" + "=" * 80)
    print("PROPERTY DETAIL SCRAPER - FOR-SALE PROPERTIES (Selenium)")
    print("=" * 80)
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_file = os.path.join(RESULTS_DIR, f"property_scrape_report_{timestamp}.json")
